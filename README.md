...
2. **Alternative Installation**:
   - If you prefer not to use Docker, you can download the [tar.gz packages](https://dl.photoprism.app/pkg/linux/README.html) and follow the instructions provided there.

### Build Instructions ###
The application can also be built from source using the provided `build.sh` script. Here’s how to use it:

- Usage: 
  ```bash
  ./scripts/build.sh [develop|race|static|debug|prod] [filename]
  ```
- Example:
  ```bash
  ./scripts/build.sh develop myphotoprism
  ```

Make sure to check the [documentation](https://docs.photoprism.app/getting-started/) for more details on installation and configuration.
